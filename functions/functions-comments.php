...
function redirect_to_thank_page($location, $commentdata)
{
  ...
  $uuid = get_comment_meta($commentdata->comment_ID, 'feedback_uuid', true);
  if ('local_services' == get_post_type($post_id)) {
    ...
    // set a cookie with the uuid in it - we need this on the next page
    setcookie('comment_uuid', $uuid, time() + HOUR_IN_SECONDS, COOKIEPATH, COOKIE_DOMAIN);
